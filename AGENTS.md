...
**Verification doctrine (V1–V9)** is the other half and the one that gets skipped, because a green report is pleasant and checking it is not: one worktree per *agent* including verifiers, mutation-first tests, brief the specific hazard, verifiers establish their own baselines, name-sets over counts, never benchmark under a fleet, keep the host awake and detect stalls by artifact, draft PRs so CI gates without auto-merge landing anything, and iteration cost as real work. Full doctrine, the motivating pattern, and the topics-manifest / scope-grant model: [`docs/orchestration.md`](docs/orchestration.md).
