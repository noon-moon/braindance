...
**This repo is TOOLING; your notes are a separate repo.** Not a product — there is no version of this to hand a stranger, and no quickstart, gates, or export. What the separation still buys is privacy, not distribution: two instances run on one machine and one of them may belong to an employer, so no personal content, no hardcoded paths, and no one operator's domain or droplet in this checkout. Before committing, ask whether it is about *how you work* (here) or *what you are working on* (your vault). See [`docs/architecture.md`](docs/architecture.md).

> **The test every change has to pass: could Obsidian do this? Could a community plugin? Could a Shortcut, or a cron?** If any answer is yes, it does not get built here. Asked once, that question would have prevented roughly nine thousand lines that were written, maintained, and then deleted — the task engine, the review desk, the vault viewer, the Today tab, the calendar feed, the capture form. Reasoning in [`docs/architecture.md`](docs/architecture.md).
...
**Single external root (`$BD_ROOT`).** The vault and the repos dir are *external resources* the core resolves off one optional knob. **The vault is always external — there is no nested default and `./configure` refuses without one.** Set `BD_ROOT` and the core, `vault/`, and repos become siblings under it (vault → `$BD_ROOT/vault`, repos → `$BD_ROOT/<name>`, agent worktrees → `$BD_ROOT/worktrees`); `VAULT_PATH` / `REPOS_PATH` / `BD_WT` are explicit per-resource overrides, each also settable per-instance in the registry conf. Scratch (`$vault/_ephemeral/`) always rides with the vault so it stays Obsidian-visible. `repo/` below names the **default** repos location; the vault has no default and is written as `$VAULT_PATH`.
...
## The vault is the working context

The vault is the canonical knowledge base — ground truth about the user's world, projects, and decisions. It resolves at `${VAULT_PATH:-$BD_ROOT/vault}` and has no fallback: unset, every tool here says so and stops rather than guessing at a path. When a task depends on that context, search the vault before acting and treat what you find there as authoritative. It is a separate repo with its own remote, synced by obsidian-git.
...
- **Tooling, not notes** — this repo is about *how you work*; what you are working on lives in the vault repo. Genericity is hygiene (no personal content, no hardcoded paths), kept for the privacy boundary between instances rather than to ship anything.
