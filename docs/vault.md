...
The vault resolves at `${VAULT_PATH:-${BD_ROOT:+$BD_ROOT/vault}}`, defaulting to `ctx/vault` inside the checkout (see [`CLAUDE.md`](../CLAUDE.md)); `ctx/vault/...` paths below name that **default** — read them as "wherever the vault resolves." Its `_ephemeral/` scratch always rides with the vault (never the core), so it stays Obsidian-visible.
