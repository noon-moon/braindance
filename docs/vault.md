...
**It never commits, and that is the whole line** — no staging, no stashing, no resetting, no `git add -A` on a timer. A vault always carries uncommitted work (a half-written note, `.obsidian/workspace.json` churning as you move panes), and sweeping that into commits you never chose is the failure worth designing against. It *does* pull and it *does* push: the deliberate act is the **commit**, choosing what goes, and once you have made one, sending it is mechanical. Refusing to send it protected nothing and stranded work on the desk — which is the failure described just above, a month of droplet captures sitting unmoved because a sync had paused and nobody was watching. So you hit commit; this publishes it on the next tick. Nothing you did not choose can leave, because nothing you did not choose is ever committed. The pull is `--ff-only`; if incoming changes touch a file you have modified, git declines before touching anything and the tool reports it and moves on — the next run succeeds once you have committed or reverted. A true divergence (local commits *and* remote commits) is reported, never merged on a timer, because that is a decision you should see. Behaviour is pinned by `test_vault-pull.sh`, whose central assertions are that an uncommitted draft comes through byte-identical and that HEAD never moves on its own.
...
**A long hold is said out loud.** Being held back is correct — git declined rather than touch your uncommitted work — but it was *silent*, so you could walk away with one uncommitted note and come back to a desk hours behind. After `BD_HELD_WARN_MIN` minutes (default 30; a few minutes of holding is just you typing) it writes `_ephemeral/!BRAINDANCE SYNC HELD.md` saying how long, how far behind, and what clears it. Deleted the moment a pull succeeds, so if it is there it is current.

It goes in `_ephemeral/` because that is **gitignored**, and that is the load-bearing part: the script's guarantee is that nothing you did not choose is ever committed, and a status file in `_triage/` would ride out on your next commit, reach the box, and need deleting and committing again to clear. Here it *cannot* be committed. It is also honest about scope — a held desk is desk-local, and the phone is fine. The fixed name is a deliberate exception to `_ephemeral`'s complemented-timestamp convention: that convention makes accumulating scratch sort newest-first, while this is a singular file that must be found and deleted by name.

**It stands aside when something else will commit the tree.** Obsidian keeps the vault in memory and writes its own copies back on its own schedule; it does not reliably notice a bulk change made underneath it. On its own that is survivable — the stale writes sit uncommitted, this tool sees a dirty tree and holds back. What makes it dangerous is obsidian-git's **auto-commit**, whose order is commit → pull → push: a stale tree is captured before any pull can correct it, so `pullBeforePush` cannot save you. That is how a 43-file frontmatter migration was reverted by a `vault backup` commit descending from the migration itself, taking generated `_meta/Topics.md` with it.

So the check is narrow and exact: *is something going to commit whatever is on disk without looking?* Obsidian running, obsidian-git enabled, **and** auto-commit on (`autoSaveInterval` or `autoBackupAfterFileChange`). Anything else — including obsidian-git pushing every minute but never committing — and it pulls as normal. **The default is to pull**: the two failures are not equal, since pulling behind an overwriter costs a revert git remembers while not pulling costs silent staleness, which is the whole reason this script exists. `BD_PULL_ANYWAY=1` overrides.
